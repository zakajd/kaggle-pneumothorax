...
from src.utils import MODEL_FROM_NAME, mask_to_polygons, eval_spacenet
...
def main(hparams):
    assert os.path.exists(hparams.path)
    # Add model parameters 
    with open(hparams.path + "/config.yaml", "r") as file:
    ...
    # Get dataloaders
    val_loader, test_loader, val_files, test_files = get_test_dataloaders(root=hparams.root)
